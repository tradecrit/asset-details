...
RUN cargo build --release --package ingestor --bin ingestor
...
COPY --from=builder /target/release/ingestor /usr/local/bin/

# Create a new user for running the service
RUN addgroup -S appuser && adduser -S appuser -G appuser

# Set ownership and permissions
RUN chown appuser:appuser /usr/local/bin/ingestor
